...
- Words, bindings, operators, expressions.
...
| word         | Word type, used for bindings         | `foo`                      | `word%{ word with spaces }`    | `word 'foo; word "bar"; word " word with spaces "`  |
...
1) `~:b` and `_:c` are read as *ordinary words*.
   `~` and `_` are *simple, word-valid characters* outside of syntax-quote or string-interpolation.
...
- `&:binding.asString`: canonical textual representation of the lexical word itself.
...
#       So there's no separate word for type assignment (type declaration + definition).
#       Meaning `let` can *always* be used to bind an expression <expr> to a word.
...
| `%`                     |          | Dispatch operator: operator implementations; bindings for words; basic literal syntax                     |
| `%deref expr`           | `*expr`  | Deref operator; realizes protected or promise-based values such as futures or laziness.                   |
| `%ref expr`             | `&expr`  | Ref operator; references the word itself. Required when working with the default (form) binding.          |
...
- operators *may* be separated from neighbour words by spaces/line-breaks (or not)
...
| ----------- | ------- | ------------ | ---------------------------------------------------------------------------------------------------------- |
|  `gra%word` | `^word` | graph        | **Transactional Memory binding**: modify *many* related identities, as a *single* transaction, at the *same* time. Internally used for *types*, *patterns*/*interfaces*, and *computations*. |
|   `fm%word` |  `word` | graph        | **Form binding**: associates a word with a *form* (*function* or *macro*).                                 |
|   `if%word` |         | graph        | **Interface binding**: describes the *signature* or *interface* for the *form* and *computation* bindings. |
|   `cp%word` |         | graph        | **Computation binding**: describes the *compute*/*body* for *form* and *interface* bindings.               |
|  `key%word` | `~word` | static       | **Keyword** or **Option binding**: *self-bound*, *global*, and *static*. **Cannot be rebound**.            |
|  `val%word` | `:word` | lexical      | **Lexical binding**: *Usually immutable* in iklo. Used for *locals*, *function arguments*, etc.            |
|  `var%word` | `$word` | dynamic      | **Variable binding**: *thread-local* identities with a *shared default*. Like clojure **vars**.            |
|   `rx%word` |         | reactive     | **Reactive binding**: *event-sourced*, *reactive* binding. Like Clojure **agents**.                        |
| `sync%word` |         | synchronised | **Entity binding**: *synchronous* and *uncoordinated*. Like Clojure **atoms**.                             |
...
1. Reader emits a flat stream of words/literals/operators with source spans.
...
- **slot grammar**: required/optional arguments and marker words between arguments.
...
This is how words "interpret themselves" without ambiguous free-form parsing.
