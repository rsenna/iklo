...
- **VI. ADRs for Load-Bearing Decisions** ✅ — grounded in [ADR-0001](../../specs/decisions/ADR-0001-substrate-boundary.md). Any new load-bearing sub-decision during implementation becomes the next unused ADR number (ADR-0002 records the cross-agent Supermemory setup; [ADR-0003](../../specs/decisions/ADR-0003-queue-evaluation-exploration.md) is an exploratory queue-evaluation note — check the specs/decisions/ directory for the current next-free number before assigning).
