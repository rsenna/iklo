...
- [x] **T003** [US1] Write RED unit tests (in `crates/iklo-cli/src/main.rs` or a new `repl.rs`, `#[cfg(test)] mod tests`) for:
  - `should_save_history(had_existing_history: bool, entries_added_this_session: bool) -> bool` — pure predicate, no I/O: `(false, false) -> false` (the critical case — a fresh session with zero input must not create a file, per spec.md Acceptance Scenario 3); `(false, true) -> true`; `(true, false) -> true` (successfully-loaded existing history is still saved even if this session added nothing, keeping behavior simple and predictable); `(true, true) -> true`.
  - A load-history smoke test using a real scratch path (e.g. `std::env::temp_dir().join(format!("iklo-test-history-{}", std::process::id()))`, cleaned up after): loading from a path that does not exist returns an `Err` (rustyline's own behavior) — the REPL must treat this as non-fatal (log/ignore, never panic or propagate), per spec Edge Cases.
...
- [x] **T005** [US1] Load history from `.iklo_history` (cwd) at REPL startup via `rl.load_history(...)`; treat a missing/unreadable/corrupt file as non-fatal (per spec Edge Cases). Record `had_existing_history` as whether `load_history` actually returned `Ok` — not merely whether the file existed — so a corrupt file that fails to load is never later overwritten with an empty history (see T006). **Acceptance**: starting the REPL with no existing history file does not error.
- [x] **T006** [US1] Call `rl.add_history_entry(...)` once per complete submitted entry — a whole multi-line expression as ONE entry when it finishes parsing (success or a genuine error, not `UnexpectedEof`), and each recognized `.quit`/`.revision`/`.env` command immediately — never per raw input line (a cancelled continuation via blank line or Ctrl-C must leave no trace, and a multi-line expression must recall as one unit, not fragments). Only flip `entries_added_this_session` when `add_history_entry` returns `Ok(true)` (rustyline may ignore duplicates). On REPL exit (`.quit`, and on EOF — slash commands land in Phase 4), call `should_save_history(had_existing_history, entries_added_this_session)` and only call `rl.save_history(...)` if it returns `true`; report (don't silently discard) a `save_history` failure. **Acceptance**: manual smoke test — Up arrow recalls prior entries within a session, a multi-line expression as one recallable entry; after quit+restart, prior session's history is still reachable; a fresh session quit with zero input leaves no `.iklo_history` file; a cancelled multi-line input is never recorded; an unreadable pre-existing history file is preserved untouched by a zero-input session.
...
- [x] **T007** [US2] Write RED unit tests for the two functions from plan.md Key Design Decision 1:
...
- [x] **T008** [US2] Implement `is_repl_command_position` and `parse_repl_command` per T007. **Acceptance**: T007's tests pass, 0 failed.

### Implementation for User Story 2

- [x] **T009** [US2] Remove the `.quit`/`.revision`/`.env` string-equality checks from `run_repl()`; replace the submit-time dispatch with: call `is_repl_command_position(buffer.is_empty(), &line)`, and only if `true`, call `parse_repl_command(&line)` and dispatch on the result — falling through to ordinary parsing on `None` at either step (matching FR-006's "unrecognized `/foo` falls through to the parser" requirement). **Acceptance**: manual smoke test — `/quit`, `/revision`, `/env` behave identically to today's `.`-commands; `.quit` etc. no longer do anything special (fall through to the parser, producing a parse error); `  /quit` (leading whitespace) is not treated as a command.
- [x] **T010** [US2] Implement a custom `Completer` (bundled into a `Helper` via rustyline's derive macros per plan.md's Technical Context) that calls `is_repl_command_position` — using the helper's own continuation-state field, updated via `rl.helper_mut()` by `run_repl()` immediately before each `rl.readline(...)` call, per plan.md Key Design Decision 2 — to decide eligibility, then does its own prefix-filtering over `["quit", "revision", "env"]` against whatever partial text follows `/` (standard rustyline completion pattern; does NOT call `parse_repl_command`, per Key Design Decision 1). Wire via `rl.set_helper(...)`. **Acceptance**: manual smoke test — typing `/` + Tab at a fresh prompt offers the three commands; typing `/q` + Tab offers `quit`; typing `/` + Tab mid-continuation offers nothing.
- [x] **T011** [US2] Manual smoke test covering spec.md's full Acceptance Scenarios list for User Story 2 (all 6 scenarios) plus Edge Cases (Ctrl-C/Ctrl-D behavior preserved; `/` mid-continuation inert; leading-whitespace `/quit` inert). **Acceptance**: every scenario behaves as specified; discrepancies get a task, not a silent skip.
  - **Verified 2026-07-21** via real-pty tests (Python `pty.fork()`, not piped stdin — Tab-completion and Ctrl-C require a real terminal): bare `/` + Tab and `/q` + Tab both offer/expand completions (decisively confirmed by `/q`+Tab+Enter exiting the REPL, which only happens if Tab expanded it to the exact string `/quit`); mid-continuation Tab emits only a terminal bell (`\x07`), zero text insertion — confirmed via raw byte comparison, not just visual inspection. `10 / 2` still evaluates as division. `/quit` mid-continuation causes a parse error (T_Slash), never an exit — and the subsequent transactional rollback correctly leaves the attempted binding undefined. Ctrl-C re-verified with no regression from PR #13's fix (state survives, buffer clears, REPL doesn't exit).
