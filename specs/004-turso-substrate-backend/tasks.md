...
| B001 | adapter-fixable | none (scoping decision, not a broken invariant) | The `turso` Rust crate (v0.7.0) has no direct "talk to remote Turso Cloud per-query" client — only a local file/`:memory:` mode (`Builder::new_local`), or an embedded-replica mode requiring an explicit local replica path plus manual `push()`/`pull()` calls (`turso::sync::Builder::new_remote`). FR-011's CLI flags (`--turso-db-url`, `--turso-auth-token`) provide only two knobs, which don't map onto the sync API's three required parameters (local path + remote url + token). | `--turso-db-url` is treated as a **local file path** for this epic (`Builder::new_local(path)`, no `sync` feature, no push/pull, `--turso-auth-token` accepted but unused/no-op for now). Fully satisfies this epic's actual acceptance criteria (SC-002: restart persistence on the same machine) without remote connectivity. | User (RG Senna) confirmed: local-file-only is sufficient: connecting to "bare"/cloud Turso databases is not a required feature for this epic. Iklo may fork/diverge from upstream Turso later and may not stay wire-compatible with Turso Cloud at all; remote sync (e.g. for connecting separate Iklo instances) is an explicit future concern, not in scope now. |
